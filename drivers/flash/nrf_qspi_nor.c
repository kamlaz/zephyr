...
#define QSPI_NOR_QE_BIT                         BIT(CONFIG_QE_BIT)
 ...
 * @sync: The semaphore to ensure that transfer has finished
 * @write_protection: Indicates if write protection for flash
 *  device is enabled
 */
...
static inline void qspi_fill_init_struct(nrfx_qspi_config_t *initstruct)
{
	...
	initstruct->prot_if.readoc = (nrf_qspi_writeoc_t)
				     qspi_get_lines_read
					     (DT_INST_0_NORDIC_QSPI_NOR_READOC_ENUM);
#else
	initstruct->prot_if.readoc = NRF_QSPI_READOC_FASTREAD;
#endif

#ifdef DT_INST_0_NORDIC_QSPI_NOR_WRITEOC_ENUM
	initstruct->prot_if.writeoc = (nrf_qspi_writeoc_t)
				      qspi_get_lines_write
					      (DT_INST_0_NORDIC_QSPI_NOR_WRITEOC_ENUM);
#else
	...
	initstruct->phy_if.spi_mode = qspi_get_mode
					      (DT_INST_0_NORDIC_QSPI_NOR_CPOL,
...
int qspi_nrfx_configure(struct device *dev)
{
	if (!dev) {
		return -ENXIO;
	}
			 ...
			 size_t size)
{
	...
	int ret = nrfx_qspi_read(rx_buf.buf, rx_buf.len, addr);
			  ...
			  size_t size)
{
	...
	int ret = nrfx_qspi_write(tx_buf.buf, tx_buf.len, addr);
...
#define LAYOUT_PAGES_COUNT (INST_0_BYTES / \
			    CONFIG_QSPI_NOR_FLASH_LAYOUT_PAGE_SIZE)

BUILD_ASSERT_MSG((CONFIG_QSPI_NOR_FLASH_LAYOUT_PAGE_SIZE *
		  LAYOUT_PAGES_COUNT)
		 == INST_0_BYTES,
